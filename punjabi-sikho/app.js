/* Punjabi Sikho v1: speaking-first prototype (same engine as Gujarati Shikho). No build step, no dependencies.
   All language-specific content lives in data/<language>.js and config.js. */
(function () {
  'use strict';

  const C = window.COURSE;
  const CFG = window.APP_CONFIG || {};
  const KEY = CFG.storageKey || 'sikho-v1';
  ...
  const nativeVoice = () => voices.find((v) => String(v.lang || '').toLowerCase().replace('_', '-').indexOf(C.voiceLang) === 0);
  refreshVoices();
  if (synth) {
  ...
  function speakTTS(text, slow, onend) {
    ...
    refreshVoices();
    const v = nativeVoice();
    if (!v) { toast('No ' + C.name + ' voice on this device. The audio files are needed here.'); return; }
    synth.cancel();
  ...
  function play(it, slow, onend) {
    ...
    if (CFG.useRecordedAudio) {
      let fell = false;
      const fallback = () => { if (!fell) { fell = true; speakTTS(it.nat, slow, onend); } };
  ...
  function homeView() {
    ...
    let h = '<header class="hero">' +
      ...
      '<h1 class="wordmark nat" lang="' + C.voiceLang + '">' + esc(C.nativeName) + '</h1>' +
    ...
    if (!CFG.useRecordedAudio && !guVoice()) {
      h += '<p class="note">' + (synth
        ? 'This device has no ' + C.name + ' voice, so the sound buttons stay silent. Audio files fix this: set useRecordedAudio to true in config.js.'
    ...
    if (CFG.showTestingTools !== false) h += '<details class="tools"' + (toolsOpen ? ' open' : '') + '><summary>Testing tools</summary>' +
      ...
      '<label class="check"><input type="checkbox" data-a="toggle-script"' + (P.showScript ? ' checked' : '') + '> Show ' + C.name + ' script</label>' +
  ...
  const scriptLine = (it) => (P.showScript ? '<p class="nat-line nat" lang="' + C.voiceLang + '">' + esc(it.nat) + '</p>' : '');
  ...
  function feedback(ex, ok) {
    ...
    if (P.showScript) body += ' <span class="nat" lang="' + C.voiceLang + '">' + esc(it.nat) + '</span>';
  ...
  function reviewView() {
    let h = '<div class="rv"><h1>Content review</h1>' +
      '<p>For a native speaker to check. Read each row and note anything wrong: the ' + C.name + ' spelling, the transliteration, the meaning, or how it sounds in the audio. Highlighted rows are the ones we are least sure about. Audio files are named from the "said as" text.</p>' +
    ...
    lessons.forEach((l) => {
      if (!l.items) return;
      h += '<h2>' + esc(l.title) + '</h2><div class="rv-scroll"><table><thead><tr><th>Said as</th><th>' + esc(C.name) + '</th><th>Meaning</th><th>Audio file</th></tr></thead><tbody>';
      l.items.forEach((it) => {
        h += '<tr' + (it.check ? ' class="flag"' : '') + '><td>' + esc(it.rom) + '</td><td class="nat" lang="' + C.voiceLang + '">' + esc(it.nat) + '</td><td>' + esc(it.en) + (it.check ? ' <strong>(please check)</strong>' : '') + '</td><td><code>' + it.id + '.mp3</code></td></tr>';
      });
      h += '</tbody></table></div>';
  ...
  function copyCsv() {
    ...
    const lines = [['audio_file', 'lesson', 'said_as', C.name.toLowerCase(), 'meaning', 'needs_native_check'].map(q).join(',')];
    lessons.forEach((l) => (l.items || []).forEach((it) => lines.push([it.id + '.mp3', l.title, it.rom, it.nat, it.en, it.check ? 'yes' : ''].map(q).join(','))));
