...
for (const d of dirs) if (!fs.existsSync(path.join(root, d))) {
  fail('Missing the "' + d + '" folder in ' + root + (d === 'audio' ? '. Run git pull so the audio files are here.' : ''));
}
...
if (mp3 < 240) console.warn('WARNING: only ' + mp3 + ' audio files found. The full course has about 252. Run make_audio.py, then git pull.');
